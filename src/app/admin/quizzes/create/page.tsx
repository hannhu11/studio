...
type GeneratedQuiz = Omit<Quiz, 'id' | 'createdAt'>;
...
export default function CreateQuizPage() {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      ...
      // The ID will be added on the server-side before saving to prevent serialization issues.
      let allQuestions: Omit<Question, 'id'>[] = results.flatMap(result => 
        result && result.questions ? result.questions : []
      );

      if (allQuestions.length > MAX_QUESTIONS_LIMIT) {
        setError(`You cannot generate more than ${MAX_QUESTIONS_LIMIT} questions at once. Please reduce the number of images or questions in the images.`);
        allQuestions = allQuestions.slice(0, MAX_QUESTIONS_LIMIT);
      }
      
      if(allQuestions.length > 0) {
        setGeneratedQuiz({ 
            questions: allQuestions as Question[], // Cast here for the UI, ID will be added on server
            title: `Generated Quiz - ${new Date().toLocaleDateString()}`,
            description: 'A new quiz generated by AI from uploaded images.'
        });
         if (allQuestions.length >= MAX_QUESTIONS_LIMIT) {
          toast({
            variant: "default",
            title: "Question Limit Reached",
            description: `The quiz has been capped at ${MAX_QUESTIONS_LIMIT} questions.`,
          });
        }
      } else {
            ...
            <CardContent>
              ...
              {generatedQuiz && (
                <div className="space-y-6">
                  ...
                  <Button size="lg" className="w-full mt-6" onClick={handlePublishQuiz} disabled={isPublishing || !generatedQuiz.title}>
